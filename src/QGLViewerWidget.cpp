...
QGLViewerWidget::init(void)
{
	...
	modelview_matrix_ = modelview_matrix_A;
...
QGLViewerWidget::initializeGL()
{
	...
	// Modified: Minhyuk Sung. 2009-10-25
	...
	glLoadIdentity();
	glGetDoublev(GL_MODELVIEW_MATRIX, modelview_matrix_A);
	glGetDoublev(GL_MODELVIEW_MATRIX, modelview_matrix_B);
...
QGLViewerWidget::paintGL()
{
	...
	if (!hide_A_)
	{
		glLoadMatrixd(modelview_matrix_A);
		set_render_view_A();
		if (draw_mode_)
		{
	...
	if (!hide_B_)
	{
		glLoadMatrixd(modelview_matrix_B);
		set_render_view_B();
		if (draw_mode_)
		{
			assert(draw_mode_ <= n_draw_modes_);
...
#ifdef MOVE_BOTH_TOGETHER
	memcpy(modelview_matrix_A, modelview_matrix_, 16 * sizeof(double));
	memcpy(modelview_matrix_B, modelview_matrix_, 16 * sizeof(double));
#endif
}
...
// void slotSnapshot( void ) -> void slotSnapshot( const char *filename )
void
QGLViewerWidget::slotSnapshot(const char *_filename)
{
	QImage image;
...
void
QGLViewerWidget::getTransformedCoord(
const GLdouble *_matrix,
const GLdouble *_coord,
GLdouble output[3])
{
	output[0] = (_matrix[0] * _coord[0] +
		_matrix[4] * _coord[1] +
		_matrix[8] * _coord[2] +
		_matrix[12]),

		output[1] = (_matrix[1] * _coord[0] +
		_matrix[5] * _coord[1] +
		_matrix[9] * _coord[2] +
		_matrix[13]),

		output[2] = (_matrix[2] * _coord[0] +
		_matrix[6] * _coord[1] +
		_matrix[10] * _coord[2] +
		_matrix[14]);
}

void QGLViewerWidget::set_modelview_matrix(GLdouble *matrix)
{
	for (unsigned int i = 0; i < 16; i++) modelview_matrix_[i] = matrix[i];
