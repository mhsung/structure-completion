...
#define OPENMESHAPPS_MESHVIEWERCORET_CPP
...
template <typename M>
bool
MeshViewerCoreT<M>::open_mesh(const char* _filename)
{
	// load mesh
	// calculate normals
	...
	//setWindowTitle(QFileInfo(_filename).fileName().toStdString());
...
/*
template <typename M>
bool MeshViewerCoreT<M>::open_texture(const char *_filename)
{
	QImage texsrc;
...
template <typename M>
bool MeshViewerCoreT<M>::set_texture(QImage& _texsrc)
{
...
template <typename M>
void
MeshViewerCoreT<M>::draw_openmesh(const std::string& _draw_mode)
{
	...
	if (_draw_mode == "OpenSG Indices") // --------------------------------------
	{
		...
		if (tex_id_ && mesh_.has_vertex_texcoords2D())
		{
			...
			glTexCoordPointer(2, GL_DOUBLE, 0, mesh_.texcoords2D());
		...
		else if (_draw_mode == "Show Strips" && strips_.is_valid()) // -------------
		{
			...
			for (; strip_it != strip_last; ++strip_it)
			{
				...
				for (; idx_it != idx_last; ++idx_it)
					glVertex3dv(&mesh_.point(OM_TYPENAME Mesh::VertexHandle(*idx_it))[0]);
				glEnd();
			}
...
template <typename M>
void
MeshViewerCoreT<M>::draw_scene(const std::string& _draw_mode)
{

	if (!mesh_.n_vertices())
	{
		GLViewerCore::draw_scene(_draw_mode);
		return;
	}
...
template <typename M>
void
MeshViewerCoreT<M>::enable_strips()
{
	if (!f_strips_)
	{
		f_strips_ = true;
...
template <typename M>
void
MeshViewerCoreT<M>::disable_strips()
{
	if (f_strips_)
	{
		f_strips_ = false;
...
template <typename M>
void
MeshViewerCoreT<M>::keyPressEvent(QKeyEvent* _event)
{
	switch (_event->key())
	{
	case Key_D:
